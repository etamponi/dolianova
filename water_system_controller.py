  ...
  def print_state(self):
    ...
    if self.state["tank1_state"] == "filling":
      ...
      if self.state["pump1_running"]:
        ...
        print(
          f"Pump1 is running for {minutes(duration)} minutes (since {pump1_start_time})"
        )
        ...
        print(
          f"Pump1 was turned off {minutes(duration)} minutes ago (at {pump1_last_on})"
        )
        print(
          f"Pump1 will be turned on again in {minutes(cooldown - duration)} minutes"
        )
      else:
        print("Pump1 is off")
    ...
    if self.state["tank1_state"] == "emptying":
      ...
      if now - tank1_filled_at < wait_time:
        if self.state["pump2_running"]:
          print(
            f"WARNING: pump2 should not be running if tank1 was filled less than {minutes(wait_time)} minutes ago"
          )
        else:
          print(
            f"Tank1 was filled {minutes(now - tank1_filled_at)} minutes ago (at {tank1_filled_at})"
          )
          print(
            f"Pump2 will be turned on in {minutes(wait_time - elapsed_time)} minutes (at {tank1_filled_at + wait_time})"
          )
